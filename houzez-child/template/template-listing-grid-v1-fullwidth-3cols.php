...
// Apply property sorting if applicable
$listing_args = apply_filters('houzez20_property_filter', $listing_args);
$listing_args = houzez_prop_sort($listing_args);

// Check if we are on the 'off-market' page and apply property type filtering
if (is_page('off-market')) {
    $tax_query = array(
        array(
    ...
    if (isset($_GET['property_type_filter'])) {
        $filter = sanitize_text_field($_GET['property_type_filter']);
...
<div class="listing-tools-wrap">
    <div class="d-flex align-items-center <?php echo esc_attr($mb); ?>">
        ...
        <?php 
        get_template_part('template-parts/listing/listing-tabs'); 
        if (!is_page('off-market')) {
            get_template_part('template-parts/listing/listing-sort-by'); 
        } else {
            echo do_shortcode('[property_type_dropdown]');
        }
        ?>
    </div><!-- d-flex -->
